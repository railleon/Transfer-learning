...
def train_model(model:Model, dataset_id, dataset_prefix, dataset_fold_id=None, epochs=50, batch_size=128, val_subset=None,
                cutoff=None, normalize_timeseries=False, learning_rate=1e-3, monitor='loss', optimization_mode='auto', compile_model=True, four_input= False,
                target=False, X_train_min=None, X_train_range=None, y_train_min=None, y_train_range=None):

    if target:
        X_train, y_train, X_test, y_test, is_timeseries, y_train_mean, y_train_std = load_dataset_at(dataset_id,
                                                                                                     fold_index=dataset_fold_id,
                                                                                                     normalize_timeseries=normalize_timeseries,
                                                                                                     target=target,
                                                                                                     X_trainmin=X_train_min,
                                                                                                     X_trainrange=X_train_range,
                                                                                                     y_trainmin=y_train_min,
                                                                                                     y_trainrange=y_train_range)
    ...
    if compile_model:
        ...
        model.compile(optimizer=optm, loss='mean_squared_error', metrics=['mean_squared_error'])
    ...
    if four_input:
        ...
        X_train=X_train.reshape(-1,max_timesteps, max_nb_variables,1)

        X_test = X_test.reshape(-1, max_timesteps, max_nb_variables, 1)
    ...
    return history,y_train_mean, y_train_std


def evaluate_model(model:Model, dataset_id,  dataset_prefix, dataset_fold_id=None, batch_size=128, test_data_subset=None,
                   cutoff=None, normalize_timeseries=False, four_input= False, predict=False,target=False, X_train_min=None, X_train_range=None, y_train_min=None, y_train_range=None):
    X_train, y_train, X_test, y_test, is_timeseries,y_train_mean, y_train_std = load_dataset_at(dataset_id,
                                                          fold_index=dataset_fold_id,
                                                          normalize_timeseries=normalize_timeseries,target=target,
                                                                                                     X_trainmin=X_train_min,
                                                                                                     X_trainrange=X_train_range,
                                                                                                     y_trainmin=y_train_min,
                                                                                                     y_trainrange=y_train_range)
    ...
    if predict is True:
        ...
        result = (model.predict(X_test))*(y_train_std )+y_train_mean
        np.savetxt('predicted result_%d.txt' %dataset_fold_id, result, fmt='%1.9f')


        result2 = y_test*(y_train_std)+y_train_mean
        np.savetxt('correct result_%d.txt'%dataset_fold_id, result2, fmt='%1.9f')


        result3 = (model.predict(X_train)) * (y_train_std) + y_train_mean
        np.savetxt('predicted result(trained)_%d.txt'%dataset_fold_id, result3, fmt='%1.9f')


        result4 = y_train * (y_train_std ) + y_train_mean
        np.savetxt('correct result(trained)_%d.txt'%dataset_fold_id, result4, fmt='%1.9f')
